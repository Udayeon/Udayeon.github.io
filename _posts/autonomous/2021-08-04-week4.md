...
감소할지라도 새로운 데이터 예측 시에는 오차가 증가하게 된다. Over-fitting으로 인해 일반화 능력이 떨어진 경우 이를 향상시키기 위해선
Bias와 분산을 적절히 선택해야한다. **Bias(편향)** 는 예측치가 이루는 오차의 평균을 의미한다. 즉, Bias가 적다는 것은 오차가 적다는 것을 의미하므로
적을수록 정답에 가깝다. **Variance(분산)** 은 퍼져있는 정도를 이야기한다. 분산이 크다는 것은, Training dataset가 바뀔 때마다 모양이 크게
요동치는 것을 의미한다. 

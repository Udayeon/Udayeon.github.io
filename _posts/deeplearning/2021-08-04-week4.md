...
일반화 능력을 향상시키는 또다른 방법은 **Data를 더 많이 수집**하는 것이다. 
...
이때, (c)처럼 data 자체가 많으면 실제 system이 가지고 있는 특성을 더 많이 보여줄 수 있으므로 정확한 모델을 학습할 수 있음
...
그런데, 데이터를 수집하는 건 비용이 들고 불가능한 경우도 있다. 따라서 다음과 같은 방법을 사용할 수 있다. 
먼저, **데이터 확대(Data Augmentation)**. 이는 Training set의 sample들을 조금씩 변환(Rotation, Warping) 시키면서 우리가 관찰할 수 있을만한 
데이터를 인위적으로 생성하는 방법이다.   
...
일반화 능력 향상의 마지막 방법은 **규제(Regularization)** 이다. 어떤 모델의 weight가 너무 크지 않도록 규제하는 것이다. 
예를들어 12차 모델의 경우 data point가 많이 없을 때 굉장히 큰 weight를 가지는데 weight가 크다는 것은 over-fitting을 유발하므로 목적함수를 조정해 
이를 규제해야 일반화 능력이 향상됨.

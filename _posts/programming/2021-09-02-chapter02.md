...
#이산푸리에 변환을 사용해 공간 도메인에서 주파수 도메인으로 변환한다.
fft = cv2.dft(image, flags =cv2.DFT_COMPLEX_OUTPUT)

#낮은 주파수가 배열의 중앙에 위치하도록 이산 푸리에 변환의 결과를 이동한다.
fft_shift = np.fft.fftshift(fft,axes=[0,1])

#고주파의 진폭을 0으로 설정하고 나머지는 그대로 둔다.
sz=25
mask = np.zeros(fft_shift.shape, np.uint8)
mask[mask.shape[0]//2 -sz:mask.shape[0]//2+sz,mask.shape[1]//2-sz:mask.shape[1]//2+sz,:] =1
fft_shift *=mask

#이산 푸리에 변환의 결과를 다시 이동시킨다
fft = np.fft.ifftshift(fft_shift,axes=[0,1])

#역이산 푸리에 변환을 사용해 필터링 된 이미지를 주파수 도메인에서 다시 공간 도메인으로 변환한다.
...
plt.show()
```
![스크린샷, 2021-09-14 14-11-52](https://user-images.githubusercontent.com/69246778/133198657-8877dc58-9aee-4b2f-9d7b-5324e3eca84d.png)
...
![스크린샷, 2021-09-14 14-19-23](https://user-images.githubusercontent.com/69246778/133199246-59372f9d-a3c1-46bb-a80d-35232592fa78.png)
...
![스크린샷, 2021-09-14 14-40-51](https://user-images.githubusercontent.com/69246778/133201335-f297df6c-7f75-478d-a66c-122c6cafabf8.png)
...
```
![스크린샷, 2021-09-14 14-55-59](https://user-images.githubusercontent.com/69246778/133202729-5048b836-fbb8-4356-8afc-5c3bcfbb1c30.png)

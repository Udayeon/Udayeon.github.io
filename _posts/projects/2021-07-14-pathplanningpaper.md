...
## Abstract
* * *
MPC에 기반한, mode에 따른 효율적인 자동전환 기술 제안
{:.message}
...
두 개의 고속도로와 도심주행 환경을 포함하여 구성된 이 환경에서, 자율주행차량(자차)은 도로가 규정한 속도로 주어진 목적지를 향해 달림.
...
**?...**
각각의 time step에서 차량은 MPC를 사용하여 유한한 수평선 경로를 계획함(??)
뭔 소리냐면, 차량이 최적화된 제어 시퀀스를 계산하고 현재 time step에 해당하는 control input을 구현함.(??어떻게뭐를..?)
이 절차는 차량이 목적지에 도달할 때 까지 예측 모델(receding horizon way??)에서 이루어짐.
...
도로 Waypoint, 차선표시 그리고 주변 차량의 현재상태 포함한 환경정보를 모두 알고 있다고 가정한다.
또한 주변 차량의 예측 상태도 정확하게 예측되었다고 가정한다.

### MPC Formulation 
MPC를 이용한 접근법은 제한적인 시간을 제약조건으로 하는 최적 제어 문제이다. (너무 먼 미래까지는 예측 X)   
N을 예측 horizon으로 두자.    
경로 계획은 **비선형**의 최적화 문제로써 공식화할 수 있다.   
위 식에서 M은 고속도로 차선의 수.   
(1b) 차량 운동학에 의한 제한조건 (상태공간방정식)   
(1c) 차량의 Actuator(선형 가속도 및 회전 속도)한계를 고려하여 실현가능한 set를 제한해줌   
(1d) 자차와 주변 차량 간의 충돌을 회피하도록 해줌. 나중에 설명함.   
목적함수는 자차의 행동을 규정하기 위해 여러 다양한 term으로 구성되어 있음.
D_
